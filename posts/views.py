...
from posts.tasks import Schedule_Posts, refresh_products
...
def dashboard(request):
    ...
    for cat in category:
        if index == len(category) - 1:
            ...
            index += 1
    ################ LEADS################
    shops_prospect = Customer.objects.filter(Situation__contains="Prospect").count()
    shops_active = Customer.objects.filter(Situation__contains="Active").count()
    try:
...
def almazaziproducts(request):
    products = AlmazadiProducts.objects.all().filter(created_post=False).order_by("-id")
...
def get_products(request):
    ...
    for link in links:
        try:
            ...
            if (
                link["add_image"]
                != "https://almazadi.com/storage/app/default/picture.jpg"
            ):
...
def create_post(request, id):
    ...
    Post.objects.create(
        imagelink=product.imagelink, category=product.category, message=product.message
    )
...
def unpublished_list(request):
    ...
    context = {"posts": page_obj, "title": "Unpublished Posts"}
...
def published_list(request):
    ...
    page_number = request.GET.get("page")
    page_obj = paginator.get_page(page_number)
    context = {"posts": page_obj, "title": "Published Posts"}
...
def publish_now(request, id):
    ...
    PublishedPost.objects.create(imagelink=post.design_link,
                                         ...
                                         fb_post_id="asdasdasdasdasd",
                                         fblink="asdasdasdasdasd")
"""
        ...
        return HttpResponse(
            ...
            headers={
                "HX-Trigger": json.dumps(
                    {
                        "close": "close",
                        "showMessage": f"Please set Timezone in your profile to publish post",
                        "type": "bg-danger",
                    }
                )
            },
...
def post_details(request, id):
    ...
    context = {"post_details": post_details, "title": "Post Details"}
...
def createpost(request):
    ...
    context = {"form": form, "submitted": submitted, "title": "Create Post"}
...
def edit_details(request, id):
    ...
    context = {"post_details": post_details, "form": form, "title": "Edit Post"}
...
def new_design(request, id):
    ...
    return HttpResponse(
        ...
        headers={
            "HX-Trigger": json.dumps(
                {"showMessage": f"Your Design Are Ready.", "type": "bg-success"}
            )
        },
    )
...
def hooks(request, *args, **kwargs):
    if request.method == "POST":
        ...
        if type == "reaction":
            ...
            if value == "add":
                add_comment(
                    post_id, comment, comment_id, comment_by, comment_by_profile
                )
                print("Comment added")
            elif value == "remove":
